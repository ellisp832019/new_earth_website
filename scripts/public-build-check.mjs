...
const files=await walk(root);let bad=[];for(const f of files){const t=await readFile(f,'utf8').catch(()=> '');for(const term of forbidden)if(t.includes(term))bad.push(`${f}: ${term}`);if(f.includes('local-assets'))bad.push(`${f}: local preview asset`)}if(bad.length){console.error(bad.join('\n'));process.exit(1)}console.log('Public build gate passed: no internal paths or blocked assets found.');
