...
const assets = [
  ...
  ['microgrowHardware', 'microgrow_hardware_public.webp', 'PUBLIC-MICROGROW'],
...
for (const [key, filename, id] of assets) {
  const sourcePath = new URL(`ASSETS/${filename === 'home_hero_new_earth_main.webp' ? '02_HOME/hero' : filename === 'home_introduction_new_earth.webp' ? '02_HOME/sections' : filename === 'blueprint_hero.webp' ? '04_BLUEPRINT/hero' : filename === 'microgrow_hardware_public.webp' ? '06_MICROGROW/evidence' : filename === 'new_earth_living_app.webp' ? '10_RESOURCES/hero' : filename === 'journal_featured_home.webp' ? '09_BLOG/featured' : filename === 'resources_featured_home.webp' || filename === 'faq_glossary_featured.webp' ? '10_RESOURCES/featured' : '11_COMMUNITY/hero'}/${filename}`, root);
  ...
  const catalogueFound = key === 'microgrowHardware' || catalogue.includes(`${key}:{id:'${id}'`) || catalogue.includes(`${key}:`);
  const pageReferenced = key === 'microgrowHardware' ? source.includes('microgrow_hardware_public.webp') : source.includes(`assets.${key}`);
