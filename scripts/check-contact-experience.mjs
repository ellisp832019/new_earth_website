...
const requiredPageSnippets = [
  ...
  ['controlled enquiry field', 'name="enquiry_type"'],
  ['privacy acknowledgement', 'name="privacy_ack"'],
];

const requiredHandlerSnippets = [
  ['post only redirect', "$_SERVER['REQUEST_METHOD'] !== 'POST'"],
  ['general env route', "'General enquiry' => 'NE_CONTACT_GENERAL'"],
  ['technical env route', "'Technical partnership' => 'NE_CONTACT_PETER'"],
  ['research env route', "'Research' => 'NE_CONTACT_PETER'"],
  ...
  ['enquiry type whitelist', 'isset($enquiryTypeMap[$enquiryType])'],
  ['subject format', "'[New Earth] ' . $enquiryType"],
